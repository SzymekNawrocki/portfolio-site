...
export async function GET(request: Request) {
  ...
  let darkVibrantBackground = "#3B82F6";

  if (data.mainImage?.asset?._ref) {
    try {

      const imageBuilder = urlFor(data.mainImage).width(500).height(630);
      imageUrl = imageBuilder.url();

      const imageAsset = await client.fetch(
        `*[_id == $assetId][0]{ metadata { palette } }`,
        { assetId: data.mainImage.asset._ref }
      );

      if (imageAsset?.metadata?.palette) {
        vibrantBackground = imageAsset.metadata.palette.vibrant?.background ?? "#3B82F6";
        darkVibrantBackground = imageAsset.metadata.palette.darkVibrant?.background ?? "#3B82F6";
      }
    } catch (error) {
